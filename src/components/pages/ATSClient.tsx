...
function CandidateSearchVisual() {
  const candidates = [
    ...
    {name:"Alex Kumar",   role:"ML Engineer",       score:88, match:"bg-brand-500"},
...
function CollabHiringVisual() {
  const team = [
    {init:"AJ", name:"Alex",  vote:"✓", color:"bg-brand-600",  voteBg:"bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400"},
    {init:"SC", name:"Sarah", vote:"✓", color:"bg-brand-500", voteBg:"bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400"},
  ...
  return (
    <div className="w-full p-3 space-y-2">
      <div className="grid grid-cols-2 gap-1.5 mb-1">
        {[{l:"Time-to-hire",v:"12d",c:"text-brand-600 dark:text-brand-300"},{l:"Offer rate",v:"22%",c:"text-brand-600 dark:text-brand-400"}].map(m=>(
      ...
      {stages.map((s,i)=>(
        <div key={i} className="space-y-0.5">
          ...
          <div className="w-full bg-gray-100 dark:bg-white/[0.05] rounded-full h-1.5">
            <div className="h-1.5 rounded-full bg-brand-500" style={{width:`${s.pct}%`,opacity:0.35+i*0.15}}/>
          </div>
        </div>
      ))}
    </div>
          ...
          {steps.map((s,i)=>(
            <div key={i} className="relative flex items-center gap-2.5">
              <div className={`absolute -left-3 w-2 h-2 rounded-full border-2 ${
                s.status==="done"    ? "bg-brand-500 border-brand-500" :
              ...
              <span className={`text-[10px] font-semibold ${
                s.status==="done" ? "text-brand-600 dark:text-brand-400" :
              ...
              {s.status==="done"   && <CheckCircle2 className="w-3 h-3 text-brand-500 ml-auto shrink-0"/>}
            </div>
          ))}
        </div>
      </div>
...
const features = [
  ...
  { icon:Users,       title:"Collaborative Hiring",     description:"Bring your whole team into the hiring process with shared notes, scorecards, and real-time collaboration.",    glow:"from-brand-400/10 to-brand-300/5 dark:from-brand-400/10 dark:to-brand-300/5",   border:"border-brand-300/25 dark:border-brand-500/20",Visual:CollabHiringVisual   },
...
const pipeline = [
  ...
  { step:"06", label:"Onboard",   description:"Seamless transition from hire to team member",     color:"from-cyan-600 to-brand-500" },
];
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{once:true}} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={s.label} variants={fadeUp}
                className="text-center p-8 rounded-2xl bg-white dark:bg-brand-800 border border-gray-100 dark:border-white/[0.07] shadow-sm">
