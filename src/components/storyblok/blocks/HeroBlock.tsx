/** CMS hero: kicker, headline, CTAs, optional image (side-by-side on large screens). */
...
export default function HeroBlock({ blok }: { blok: Blok }) {
  ...
  const imgAlt = blok.image_alt?.trim() || storyblokImageAlt(blok.image, blok.title || "Hero illustration");
  ...
  return (
    <div
      className={cn(
        ...
        imgSrc && "lg:grid lg:grid-cols-2 lg:items-center lg:gap-10"
      )}
      {...storyblokEditable(blok as never)}
    >
      <div className={cn(centered && !imgSrc && "text-center", imgSrc && centered && "text-center lg:text-left")}>
        {blok.kicker && (
        ...
        <div
          className={cn(
            ...
            centered && !imgSrc && "justify-center",
            imgSrc && centered && "justify-center lg:justify-start"
          )}
        >
      ...
      {imgSrc ? (
        <div className="relative mx-auto mt-10 aspect-[4/3] w-full max-w-lg lg:mx-0 lg:mt-0 lg:max-w-none">
          <Image
            src={imgSrc}
            alt={imgAlt}
            fill
            className="rounded-2xl border border-gray-100 object-cover shadow-lg dark:border-white/[0.08]"
