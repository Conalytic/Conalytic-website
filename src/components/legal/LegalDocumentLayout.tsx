      ...
      <header className="sticky top-0 z-30 border-b border-white/60 bg-white/75 backdrop-blur-xl">
        ...
        <div className="mx-auto flex h-[3.5rem] max-w-[1400px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          >
            <Link
              href={otherHref}
              className="hidden text-sm font-medium text-[#1D2939]/70 underline-offset-4 transition-colors hover:text-[var(--brand-ink)] sm:inline"
            >
              {otherLabel}
            </Link>
            <Link
        ...
        <div className="legal-scroll-hide flex gap-2 overflow-x-auto px-4 py-3">
          {navItems.map((item) => (
            <button
              ...
              className={cn(
                "shrink-0 rounded-full border px-3.5 py-1.5 text-left text-xs font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-ink)] focus-visible:ring-offset-2",
                activeId === item.id
                  ? "border-transparent shadow-md"
                  : "border-black/10 bg-white text-[#1D2939]/80 hover:border-[var(--brand-ink)]/35",
              )}
              style={
                activeId === item.id
                  ? {
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  onClick={() => scrollToId(item.id)}
                  className={cn(
                    "relative flex w-full items-center rounded-xl py-2.5 pl-4 pr-3 text-left text-[0.8125rem] font-medium leading-snug transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand-ink)] focus-visible:ring-offset-2",
                    active
                      ? "text-[#17191C]"
