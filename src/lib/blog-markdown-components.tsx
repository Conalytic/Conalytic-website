...
/** Mutable counter for assigning h2 ids during a single markdown render pass. */
export type H2IndexCounter = { value: number };

export function createBlogMarkdownComponents(
  headingIds: string[],
  counter: H2IndexCounter,
  h2StartIndex = 0,
): Components {
  return {
    h2: ({ children, ...props }) => {
      const id = headingIds[h2StartIndex + counter.value++] ?? undefined;
      return (
        <h2
          id={id}
    ...
    p: ({ children, ...props }) => (
      <p
        className="mb-5 text-base leading-[1.75] text-gray-600 dark:text-white/72 sm:text-[1.0625rem] sm:leading-[1.8]"
        {...props}
      >
        {children}
      </p>
    ...
    ul: ({ children, ...props }) => (
      <ul
        className="mb-6 list-none space-y-2.5 pl-0 [&>li]:relative [&>li]:pl-6 [&>li]:before:absolute [&>li]:before:left-0 [&>li]:before:top-[0.65em] [&>li]:before:h-1.5 [&>li]:before:w-1.5 [&>li]:before:rounded-full [&>li]:before:bg-brand-500 [&>li]:before:content-['']"
        {...props}
      >
        {children}
      </ul>
    ...
    ol: ({ children, ...props }) => (
      <ol
        className="mb-6 list-decimal space-y-2.5 pl-6 text-[1.0625rem] leading-[1.75] text-gray-600 dark:text-white/72"
        {...props}
      >
        {children}
      </ol>
